...
import { getPreventionRisquesProfessionnelsContent } from "@/lib/content/prevention-risques-professionnels";
import { Container } from "@/components/layout/Container";

type Props = PageProps<"/[locale]/prestations/prevention-risques-professionnels">;

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { locale } = await params;
  const content = await getPreventionRisquesProfessionnelsContent(locale);
  return buildMetadata({
    title: content.seo.metaTitle,
    description: content.seo.metaDescription,
    path: "/prestations/prevention-risques-professionnels",
  });
}

export default async function PreventionRisquesProfessionnelsPage({ params }: Props) {
  const { locale } = await params;
  const content = await getPreventionRisquesProfessionnelsContent(locale);
