...
type Props = PageProps<"/[locale]/prestations/pension-survivants">;

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { locale } = await params;
  const content = await getPensionSurvivantsContent(locale);
  return buildMetadata({
    title: content.seo.metaTitle,
    description: content.seo.metaDescription,
    path: "/prestations/pension-survivants",
  });
}

export default async function PensionSurvivantsPage({ params }: Props) {
  const { locale } = await params;
  const content = await getPensionSurvivantsContent(locale);
