...
type Props = PageProps<"/[locale]/prestations/conge-maternite">;

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { locale } = await params;
  const content = await getCongeMaterniteContent(locale);
  return buildMetadata({
    title: content.seo.metaTitle,
    description: content.seo.metaDescription,
    path: "/prestations/conge-maternite",
  });
}

export default async function CongeMaternitePage({ params }: Props) {
  const { locale } = await params;
  const content = await getCongeMaterniteContent(locale);
