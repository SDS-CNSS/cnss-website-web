...
type Props = PageProps<"/[locale]/prestations/action-sanitaire-sociale">;

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { locale } = await params;
  const content = await getActionSanitaireSocialeContent(locale);
  return buildMetadata({
    title: content.seo.metaTitle,
    description: content.seo.metaDescription,
    path: "/prestations/action-sanitaire-sociale",
  });
}

export default async function ActionSanitaireSocialePage({ params }: Props) {
  const { locale } = await params;
  const content = await getActionSanitaireSocialeContent(locale);
