...
type Props = PageProps<"/[locale]/simulateurs/allocation-survivants">;

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { locale } = await params;
  const content = await getAllocationSurvivantsContent(locale);
  return buildMetadata({
    title: content.seo.metaTitle,
    description: content.seo.metaDescription,
    path: "/simulateurs/allocation-survivants",
  });
}

export default async function AllocationSurvivantsPage({ params }: Props) {
  const { locale } = await params;
  const content = await getAllocationSurvivantsContent(locale);
